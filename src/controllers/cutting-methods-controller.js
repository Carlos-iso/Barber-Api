...
exports.uploadFile = async (req, res) => {
	try {
		...
		const upload = {
			...
			id: req.body.id,
			label: req.body.label, // CuttingMethods usa label
			icon: req.body.icon,
			backgroundImage: {
				// CuttingMethods usa backgroundImage
				url: url,
				type: type,
				key: key,
			},
...
exports.update = async (req, res) => {
	try {
		...
		if (req.file) {
			...
			if (currentItem.backgroundImage && currentItem.backgroundImage.key) {
				await deleteFromR2(currentItem.backgroundImage.key);
			}
		}
...
exports.delete = async (req, res) => {
	try {
		...
		if (deletedItem.backgroundImage && deletedItem.backgroundImage.key) {
			await deleteFromR2(deletedItem.backgroundImage.key);
		}
